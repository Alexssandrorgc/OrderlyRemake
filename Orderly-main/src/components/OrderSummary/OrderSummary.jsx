  ...
  return (
    ...
    >
        ...
        {orders.map((order) => (
          ...
          >
            ...
            <IconButton
              size="small"
              color="error"
              onClick={() => onRemoveDish(order.name)}
            >
              <DeleteIcon />
            </IconButton>
          </Box>
      ...
      <Button
        ...
        disabled={!orderData.mesero || orders.length === 0} // Deshabilita si no hay nombre o no hay platillos
        style={{
          backgroundColor:
            !orderData.mesero || orders.length === 0 ? "#757575" : "#1976d2", // Gris oscuro cuando está deshabilitado
          color: !orderData.mesero || orders.length === 0 ? "#bdbdbd" : "#fff", // Color de texto gris cuando está deshabilitado
        }}
      >
        Enviar orden
      </Button>
      {(!orderData.mesero || orders.length === 0) && (
        <Typography variant="body2" color="error" sx={{ mt: 2 }}>
          Por favor, ingrese el nombre del mesero y seleccione al menos un
          platillo para poder realizar la orden.
        </Typography>
      )}
    </Paper>
  );
